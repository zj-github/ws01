...
//	}
	...
	public CrawlDatum fetch() {
		...
		try {
			...
			String responseClazz = Context.getInstance().getDetailPage(firstUrl.getDetailId()).getResponseClazz();
			Object newInstance = Class.forName(responseClazz).newInstance();
		...
		// }
		// ********* 抓取网页 **********
		strategy.fetchStrategy(firstUrl, crawlDatum);
		return crawlDatum;
	}
